  ...
  return (
    <div className="min-h-screen bg-[#0A0A0A] text-white overflow-y-auto">
      <div className="max-w-2xl mx-auto px-6 py-8 space-y-4">
        ...
        <SectionCard title="Informations générales" icon={User}>
          <div className="space-y-4">
            ...
            <Field
              ...
              helper="Numéro affiché aux clients — différent du numéro IA ci-dessous"
            >
        ...
        >
          <div className="space-y-3">
            ...
            <p className="text-xs text-gray-600 flex items-center gap-1.5">
              <Info size={11} />
              Ce numéro est géré automatiquement par TableNow. C'est celui que vous communiquez à vos clients pour les réservations par téléphone.
            </p>
          </div>
        </SectionCard>
        ...
        {/* 4. Horaires */}
        <SectionCard title="Horaires d'ouverture" icon={Globe}>
          <div className="space-y-3">
            {DAYS.map(({ key, label }) => {
              ...
              return (
                <div key={key} className="flex items-center gap-3">
                  <button
                    ...
                    className={`w-9 h-5 rounded-full transition-colors flex-shrink-0 relative ${day.open ? 'bg-green-500' : 'bg-[#222]'}`}
                  >
                    <div className={`absolute top-0.5 w-4 h-4 rounded-full bg-white transition-all ${day.open ? 'left-4' : 'left-0.5'}`} />
                  </button>
                  <span className={`text-sm w-20 flex-shrink-0 ${day.open ? 'text-white' : 'text-gray-600'}`}>{label}</span>
                  {day.open ? (
                    <div className="flex items-center gap-2 flex-1">
                      <input
                        ...
                        className="flex-1 px-2 py-1.5 rounded-lg bg-[#0f0f0f] border border-[#222] text-white text-xs focus:outline-none focus:border-green-500/50"
                      />
          ...
          <SaveButton loading={savingSection === 'schedule'} onClick={() => saveSection('schedule', { opening_hours: hours })} />
        </SectionCard>
            ...
            <div className="p-4 rounded-xl bg-[#0f0f0f] border border-[#1a1a1a] space-y-3">
              <div className="flex items-center gap-3">
                <button
                  ...
                  className={`w-9 h-5 rounded-full transition-colors flex-shrink-0 relative ${services.lunch.active ? 'bg-green-500' : 'bg-[#222]'}`}
                >
                  <div className={`absolute top-0.5 w-4 h-4 rounded-full bg-white transition-all ${services.lunch.active ? 'left-4' : 'left-0.5'}`} />
                </button>
              ...
              {services.lunch.active && (
                <div className="grid grid-cols-3 gap-3">
                  <Field label="De">
                    <input type="time" value={services.lunch.from}
                      onChange={(e) => setServices({ ...services, lunch: { ...services.lunch, from: e.target.value } })}
                      className="w-full px-2.5 py-2 rounded-xl bg-[#111] border border-[#222] text-white text-sm focus:outline-none focus:border-green-500/50" />
                  </Field>
                  <Field label="À">
                    <input type="time" value={services.lunch.to}
                      onChange={(e) => setServices({ ...services, lunch: { ...services.lunch, to: e.target.value } })}
                      className="w-full px-2.5 py-2 rounded-xl bg-[#111] border border-[#222] text-white text-sm focus:outline-none focus:border-green-500/50" />
                  </Field>
                  <Field label="Couverts max">
                    <Input value={String(services.lunch.capacity)}
                      onChange={(v) => setServices({ ...services, lunch: { ...services.lunch, capacity: parseInt(v) || 0 } })}
                      type="number" />
                  </Field>
                </div>
              )}
            ...
            <div className="p-4 rounded-xl bg-[#0f0f0f] border border-[#1a1a1a] space-y-3">
              <div className="flex items-center gap-3">
                <button
                  ...
                  className={`w-9 h-5 rounded-full transition-colors flex-shrink-0 relative ${services.dinner.active ? 'bg-green-500' : 'bg-[#222]'}`}
                >
                  <div className={`absolute top-0.5 w-4 h-4 rounded-full bg-white transition-all ${services.dinner.active ? 'left-4' : 'left-0.5'}`} />
                </button>
              ...
              {services.dinner.active && (
                <div className="grid grid-cols-3 gap-3">
                  <Field label="De">
                    <input type="time" value={services.dinner.from}
                      onChange={(e) => setServices({ ...services, dinner: { ...services.dinner, from: e.target.value } })}
                      className="w-full px-2.5 py-2 rounded-xl bg-[#111] border border-[#222] text-white text-sm focus:outline-none focus:border-green-500/50" />
                  </Field>
                  <Field label="À">
                    <input type="time" value={services.dinner.to}
                      onChange={(e) => setServices({ ...services, dinner: { ...services.dinner, to: e.target.value } })}
                      className="w-full px-2.5 py-2 rounded-xl bg-[#111] border border-[#222] text-white text-sm focus:outline-none focus:border-green-500/50" />
                  </Field>
                  <Field label="Couverts max">
                    <Input value={String(services.dinner.capacity)}
                      onChange={(v) => setServices({ ...services, dinner: { ...services.dinner, capacity: parseInt(v) || 0 } })}
                      type="number" />
                  </Field>
                </div>
              )}
        ...
        <SectionCard title="Notifications" icon={Bell}>
          <div className="space-y-4">
            <Field
              ...
              helper="Les confirmations de réservation sont envoyées à cette adresse"
            >
        ...
        >
          <div className="space-y-3">
            ...
            {user?.vapi_assistant_id && (
              <CopyField label="Assistant ID" value={user.vapi_assistant_id} />
            )}
          </div>
        </SectionCard>
